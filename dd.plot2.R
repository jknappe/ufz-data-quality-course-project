...
# mvoutlier::dd.plot()
# Mick Wu <mick.wu@ufz.de>
# 2021-02-02

# Changes:
# * Reported outliers now follow the specified alpha (without the minimum of 0.025)
# * turned of plotting

dd.plot2 <- function (x, quan = 1/2, alpha = 0.025, ...) 
{
  ...
  #            pch = lpch[j], cex = lcex[j], col = rbcol[rd >= 
  #                                                        alpha[j]])
  #   }
  #   if (j > 1 & j < lalpha) 
  ...
  #                                                        alpha[j - 1] & rd >= alpha[j]])
  #     points(distcla[rd < alpha[j]], distrob[rd < alpha[j]], 
  #            pch = lpch[j + 1], cex = lcex[j + 1], col = rbcol[rd < 
  #                                                                alpha[j]])
  #   }
  # }
